...
int main(void)
{
  ...
  /* czekamy na zakończenie poprzedniego pomiaru
  //kalibracja prztwornika ADC
  ...
  while (1)
  {
	  ...
	  HAL_Delay(250);
	  */
  //wersja bez czekania - tryb ciągły
  /*
  HAL_ADCEx_Calibration_Start(&hadc1, ADC_SINGLE_ENDED);
  HAL_ADC_Start(&hadc1);
  ...
  while (1)
  {
    ...
    HAL_Delay(250);
    */
  //2 kanały pomiaru (bez trybu ciągłego), wymaga sporo pracy ze strony procesora
  /*
  HAL_ADCEx_Calibration_Start(&hadc1, ADC_SINGLE_ENDED);
  ...
  while (1)
  {
    ...
    HAL_Delay(250);
    */
  //DMA, czyli Direct Memory Access - jakby dodatkowy prcesor, który kopiuje dane z ADC do pamięci lub gdzieś indziej
  /*
  ...
  //DAC - Przetwornik cyfrowo-analogowy oraz komparatora COMP1
